...
export function mtouch_events(d3) {
  const event = d3_eventDispatch(
		d3, mtouch, 'tap', 'dbltap', 'hold', 'drag', 'mdrag', 'touch', 'release'
	)
  let fingers = [] // array of augmented touches = fingers
  const id2finger = {} // maps ids to fingers
  let last_taps = [] // [{timeStamp: xxx, pos: [x,y]}, ...], used to detect dbltaps
  const mouse_id = 'mouse'
  const tap_max_time = 250
  const tap_max_dist2 = 10 * 10
  const hold_time = 500
  const hold_max_dist2 = 10 * 10
  const dbltap_max_delay = 400
  const dbltap_max_dist = 20
  ...
  function touchstarted() {
    ...
    const target = this
    const event_ = event.of(target, arguments)
    const touches = get_changed_touches()
  ...
  Finger.prototype.move = function(event) {
    ...
    const p = get_position(this.parent, this.id)
    const t = d3.event.timeStamp
...
function d3_eventDispatch(d3, target) {
  const dispatch = d3.dispatch.apply(this, Array.apply(null, arguments).slice(2))
